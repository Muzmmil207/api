...
class CSSBeautifierView(APIView):
    def post(self, request: Request):
        ...
        if not css_content:
            return Response(
                {"content": "Missing CSS content"},
            )

        # Use cssbeautifier to beautify the CSS
        beautified_css = beautify(
            css_content,
            {
                "indent_size": 1,
...
class JavaScriptMinifierView(APIView):
    def post(self, request: Request):
        ...
        if not javascript_content:
            return Response(
                {"content": "Missing JavaScript content"},
            )
...
class JSONToTextView(APIView):
    def post(self, request: Request):
        ...
        text_data = self.convert_json_to_text_basic(
            data
        )  # Basic approach (replace with your chosen method)
...
class CSVToJSONView(APIView):
    def post(self, request: Request):
        ...
        try:
            ...
            return Response(
                {"content": f"Invalid CSV data: {str(e)}"},
            )
...
class JSONToCSVView(APIView):
    def post(self, request: Request):
        ...
        if not isinstance(data, list) or not all(isinstance(item, dict) for item in data):
            return Response(
                {"content": "Invalid JSON format. Expecting a list of dictionaries."},
            )
